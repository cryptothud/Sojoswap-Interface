import { Currency, CurrencyAmount, Pair, Percent, Token } from '@uniswap/sdk'
...
export function useDerivedBurnInfo(
  ...
  // user specified a specific amount of liquidity tokens
    ...
    if (pair?.liquidityToken) {
      const independentAmount = chainId && tryParseAmount(chainId, typedValue, pair.liquidityToken)
  ...
  // user specified a specific amount of token a or b
    ...
    if (tokens[independentField]) {
      const independentAmount = chainId && tryParseAmount(chainId, typedValue, tokens[independentField])

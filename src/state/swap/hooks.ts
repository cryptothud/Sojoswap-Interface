...
import { Currency, CurrencyAmount, Ether, NativeCurrency, Token, Trade, TradeType } from '@uniswap/sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency instanceof NativeCurrency ? 'ETH' : ''
        })
      )
    },
    [dispatch]
  ...
  try {
    ...
    if (typedValueParsed !== '0') {
      return currency instanceof Token
        ...
        : CurrencyAmount.fromRawAmount(Ether.onChain(chainId), JSBI.BigInt(typedValueParsed))
    }
  } catch (error) {
...
export function useDerivedSwapInfo(): {
  ...
  console.log({inputCurrency, inputCurrencyId, outputCurrency, outputCurrencyId, parsedAmount, typedValue})
